...
def main():
    ...
    try:
        ...
        bpy.ops.filelink.scan_orphans("EXEC_DEFAULT", purge_orphans=False)   # f4
        bpy.ops.filelink.material_dedup("EXEC_DEFAULT", apply=False)         # f3

        checks.append(("f4 persisted", bool(getattr(wm, "filelink_rep_f4"))))
        checks.append(("f3 persisted (after f4)", bool(getattr(wm, "filelink_rep_f3"))))
        ...
        checks.append(("rows materialised for active (f3)",
                       len(wm.filelink_report_rows) == expected_rows("f3") > 0))

        wm.filelink_active_report = "f4"
        ...
        checks.append(("rows rebuilt on selector switch",
                       len(wm.filelink_report_rows) == expected_rows("f4")))

        rep = Report.from_json(getattr(wm, "filelink_rep_f4"))
        first = tree_mod.report_to_tree(rep)[0].key
        rows_collapsed = len(wm.filelink_report_rows)
        before = first in store.get_expanded(wm, store.exp_prop("f4"))
        bpy.ops.filelink.row_toggle(key=first, prop=store.exp_prop("f4"))
        ...
        checks.append(("expanding a category grows the row collection",
                       len(wm.filelink_report_rows) == expected_rows("f4") > rows_collapsed))

        out = os.path.join(tempfile.mkdtemp(), "rep.txt")
        bpy.ops.filelink.export_report("EXEC_DEFAULT", source="report", filepath=out)
        checks.append(("export wrote a file", os.path.isfile(out) and os.path.getsize(out) > 0))

        bpy.ops.object.select_all(action="DESELECT")
        r = bpy.ops.filelink.select_datablock(type="Material", name="WoodA")
        ...
        wm.filelink_active_report = remaining[0][0] if remaining else ""
        store.rebuild_report_rows(wm)
        checks.append(("clear removes active but keeps the other",
                       not getattr(wm, "filelink_rep_f4") and bool(getattr(wm, "filelink_rep_f3"))))
        checks.append(("rows rebuilt to remaining report after clear",
                       store.active_feature(wm) == "f3"
                       and len(wm.filelink_report_rows) == expected_rows("f3")))
        ...
        # single "filelink_detail_expanded" WM prop across every feature —
        ...
        bpy.ops.filelink.check_materials("EXEC_DEFAULT")
        ...
        bpy.ops.filelink.row_toggle(key=toggle_key, prop="filelink_detail_expanded")
        store.rebuild_inline_detail_rows(
            wm, "matdiag", mat_nodes, store.get_expanded(wm, "filelink_detail_expanded"))
        ...
        # FILELINK_UL_tree via the SAME "f4" inline-rows collection every
        # other section uses. Two structurally-identical, unused, fake-user
        # materials are simultaneously "fake_only" AND clustered as
        # "identical" -- real material fixtures, real filelink.scan_orphans.
        ...
        for opname in ("scan_datablock_dups", "material_dedup", "instance_geometry",
            ...
            checks.append((f"filelink.{opname} is registered",
                           hasattr(bpy.ops.filelink, opname)))
        ...
        bpy.ops.filelink.scan_datablock_dups("EXEC_DEFAULT")
        ...
        wm.filelink_resource_tree = tree_mod.nodes_to_json(rnodes)
        wm.filelink_resource_expanded = "g"
        store.rebuild_resource_rows(wm)
        checks.append(("resource rows materialised (parent + child)",
                       len(wm.filelink_resource_rows) == 2))
